...
void snow::circle() {
	...
	if (position.y <= 10 || position.y <= 800 && position.y >= 780) {
		circlesize += 3;

		ofDrawCylinder(position.x, 0, position.z, circlesize, 1);
		std::cout << "value:" << circlesize << endl;
	}
	if (position.y < 780 && position.y >= 600) {
		float circlesize = 1;
	}
